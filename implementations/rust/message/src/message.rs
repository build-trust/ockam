  ...
  impl Default for Message {
    fn default() -> Message {
      Message { version: WireProtocolVersion {v: 1},
        route: Route{onward_addresses:vec![], return_addresses:vec![]},
...
mod tests {
  ...
  #[test]
  fn route_codec() {
    let mut route: Route = Route{ onward_addresses:vec![], return_addresses:vec![] };
    route.onward_addresses.push(Address::UdpAddress(IpAddr::V4(Ipv4Addr::new(127,0,0,1)), 0x8080));
    route.onward_addresses.push(Address::UdpAddress(IpAddr::V4(Ipv4Addr::new(10,0,1,10)), 0x7070));
    route.onward_addresses.push(Address::LocalAddress( LocalAddress{ length: 4, address:vec![0,1,2,3] } ));
    let mut v: Vec<u8> = vec![];
    Route::encode(&mut route, &mut v);
    assert_eq!(v, vec![3,2,0,127,0,0,1,0x80,0x80,2,0,10,0,1,10,0x70,0x70,0,4,0,1,2,3,0]);
    match Route::decode(&v) {
      Ok((r, u)) => {
        assert_eq!(r.onward_addresses.len(), 3);
        assert_eq!(r.onward_addresses[0], Address::UdpAddress(IpAddr::V4(Ipv4Addr::new(127,0,0,1)), 0x8080));
        assert_eq!(r.onward_addresses[1], Address::UdpAddress(IpAddr::V4(Ipv4Addr::new(10,0,1,10)), 0x7070));
        assert_eq!(r.onward_addresses[2], Address::LocalAddress( LocalAddress{ length: 4, address:vec![0,1,2,3] } ));
        assert_eq!(v.len(), 24);
  ...
  fn message_codec() {
    ...
    onward_addresses.push(Address::LocalAddress( LocalAddress{ length: 4, address:vec![0, 1, 2, 9] } ));
    ...
    return_addresses.push(Address::LocalAddress( LocalAddress{ length: 4, address:vec![1, 2, 3, 4] } ));
    let route = Route{onward_addresses, return_addresses};
    ...
    match Message::decode(&u) {
      Ok((m,v)) => {
        ...
        assert_eq!(m.route.onward_addresses[0], Address::UdpAddress(IpAddr::V4(Ipv4Addr::new(127, 0, 0, 1)), 0x8080));
        assert_eq!(m.route.onward_addresses[1], Address::UdpAddress(IpAddr::V4(Ipv4Addr::new(10, 0, 1, 10)), 0x7070));
        assert_eq!(m.route.onward_addresses[2], Address::LocalAddress( LocalAddress{ length: 4, address:vec![0, 1, 2, 9] } ));
        assert_eq!(m.route.return_addresses.len(), 3);
        assert_eq!(m.route.return_addresses[0], Address::UdpAddress(IpAddr::V4(Ipv4Addr::new(127, 0, 0, 2)), 0x8080));
        assert_eq!(m.route.return_addresses[1], Address::UdpAddress(IpAddr::V4(Ipv4Addr::new(10, 0, 1, 11)), 0x7070));
        assert_eq!(m.route.return_addresses[2], Address::LocalAddress( LocalAddress{ length: 4, address:vec![1, 2, 3, 4] } ));
