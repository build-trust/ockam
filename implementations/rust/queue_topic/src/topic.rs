...
#[cfg(test)]
mod topic_tests {
    use crate::queue::{MemQueue, MemQueueWorker, QueueMessage};
    ...
    fn test_subscription_queue() {
        ...
        sub.queue
            .borrow_mut()
            .enqueue(QueueMessage::from_str("a").unwrap());
        assert!(sub.queue.borrow().has_messages())
    }
    ...
    fn test_topic_worker_publish() {
        ...
        topic_worker.publish("a", QueueMessage::from_str(validation).unwrap());
    ...
    fn test_topic_worker_unsubscribe() {
        ...
        topic_worker.publish(topic, QueueMessage::from_str("no subscribers").unwrap());
    ...
    fn test_topic_worker_consume_messages() {
        ...
        let limit = 100;
        for i in 0..limit {
            topic_worker.publish(topic, QueueMessage::from_str(&i.to_string()).unwrap())
        }
