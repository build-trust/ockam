...
impl X3dhResponder {
    fn new(v: Arc<Mutex<dyn DynVault + Send>>, identity_key: Option<Box<dyn Secret>>) -> Self {
        Self {
            identity_key,
...
impl NewKeyExchanger<X3dhInitiator, X3dhResponder> for X3dhNewKeyExchanger {
    fn initiator(&self, identity_key: Option<Box<dyn Secret>>) -> X3dhInitiator {
        X3dhInitiator::new(self.vault_initiator.clone(), identity_key)
    }

    fn responder(&self, identity_key: Option<Box<dyn Secret>>) -> X3dhResponder {
