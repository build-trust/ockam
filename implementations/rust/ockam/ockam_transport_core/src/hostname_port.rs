...
impl From<StaticHostnamePort> for HostnamePort {
    fn from(value: StaticHostnamePort) -> Self {
        Self::new(value.hostname.to_string(), value.port)
    }
}

/// Hostname and port
#[derive(Debug, Clone, PartialEq, Eq, Encode, Decode, CborLen, Serialize)]
#[rustfmt::skip]
...
impl TryFrom<SocketAddr> for HostnamePort {
    type Error = ockam_core::Error;

    fn try_from(value: SocketAddr) -> Result<Self, Self::Error> {
        HostnamePort::from_str(&value.to_string())
    }
}
...
impl FromStr for HostnamePort {
    ...
    fn from_str(hostname_port: &str) -> ockam_core::Result<HostnamePort> {
        ...
        if let Ok(socket) = parse_socket_addr(hostname_port) {
            ...
            return Ok(HostnamePort::new(ip, socket.port()));
        }
...
mod tests {
    ...
    fn test_hostname_port() -> ockam_core::Result<()> {
        ...
        let actual = HostnamePort::try_from(socket_addr)?;
        assert_eq!(
            actual,
            HostnamePort::new("[2001:db8:85a3::8a2e:370:7334]", 8080)
        );
