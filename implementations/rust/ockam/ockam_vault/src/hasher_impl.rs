...
use ockam_core::vault::{
    Hasher, Key, KeyAttributes, KeyId, KeyType, KeyVault, PrivateKey, AES128_SECRET_LENGTH_USIZE,
...
impl Hasher for Vault {
    ...
    async fn hkdf_sha256(
        ...
        info: &[u8],
        ikm: Option<&KeyId>,
        output_attributes: Vec<KeyAttributes>,
    ) -> Result<Vec<KeyId>> {
        ...
        let ikm: Result<&[u8]> = match ikm {
            Some(ikm) => {
                ...
                if ikm.key_attributes().stype() == KeyType::Buffer {
                    Ok(ikm.key().try_as_key()?.as_ref())
                } else {
        ...
        if salt.key_attributes().stype() != KeyType::Buffer {
        ...
        let okm = {
            ...
            let prk = hkdf::Hkdf::<Sha256>::new(Some(salt.key().try_as_key()?.as_ref()), ikm);
        ...
        for attributes in output_attributes {
            ...
            if attributes.stype() == KeyType::Aes {
                if length != AES256_SECRET_LENGTH_USIZE && length != AES128_SECRET_LENGTH_USIZE {
                    return Err(VaultError::InvalidAesKeyLength.into());
                }
            } else if attributes.stype() != KeyType::Buffer {
                return Err(VaultError::InvalidHkdfOutputType.into());
            }
            let secret = Key::Key(PrivateKey::new(okm[index..index + length].to_vec()));
            let secret = self.import_key(secret, attributes).await?;
