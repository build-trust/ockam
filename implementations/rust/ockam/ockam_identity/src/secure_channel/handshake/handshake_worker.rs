...
impl HandshakeWorker {
    ...
    async fn handle_handshake(
        ...
        // FIXME
        if let Some(final_state) = self.state_machine.as_ref().unwrap().get_handshake_results() {
    ...
    async fn persist(&self, their_identifier: Identifier, decryption_key: AeadSecretKeyHandle) {
        ...
        let decryption_key = match self
            ...
            .export_aead_key(&decryption_key)
            .await
        {
            Ok(decryption_key) => decryption_key,
            Err(err) => {
                warn!(
                    "Error exporting secure channel key: {err}. Local: {}, Remote: {}",
                    self.addresses.encryptor, &self.addresses.decryptor_remote
                );
