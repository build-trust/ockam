...
- Add timeout to `zone inlet/outlet` creation when waiting for the portal to be up
- In `zone init` add support to use an example directory as input
- In the zone config, transform plain env vars into the expected k8's format
- Updated dependencies

### Changed

- In `zone deploy` delete zone after processing images, instead of doing it in parallel

### Fixed

- Output for `zone list` when there are no zones for a cluster
...
- Add minimum bats tests for python examples
- Substitute env var in zone name field in zone config
- Add identity import/export commands
- Updated dependencies
