...
async fn start_authority_node(
    ...
    let identifier = match opts.state.get_identifier_by_name(&identity_name).await.ok() {
    ...
    let node = NodeInfo::new(
        ...
        false,
        true,
        Some(cmd.tcp_listener_address.clone()),
        Some(process::id()),
    );
    opts.state.store_node(&node).await?;
