...
impl Default for CreateCommand {
    fn default() -> Self {
        Self {
            ...
            child_process: false,
            launch_config: None,
            project_opts: ProjectOpts::default(),
            vault: None,
            identity: None,
...
async fn run_foreground_node(
    ...
    let (trust_context, project_addr) = match cmd.project_opts.trust_context(None) {
        None => (None, None),
        Some(cfg) => (
...
async fn spawn_background_node(
    ...
    spawn_node(
        ...
        cmd.project_opts.project_path.as_ref(),
        cmd.project_opts
            .trust_context
            .as_ref()
            .map(|t| serde_json::to_string(t).unwrap()),
        cmd.project_opts.project.as_ref(),
