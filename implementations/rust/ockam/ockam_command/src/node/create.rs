...
async fn run_foreground_node(
    ...
    let node_man = NodeManager::create(
        &ctx,
        NodeManagerGeneralOptions::new(node_name.clone(), cmd.launch_config.is_some()),
    ...
    refresh_projects(&ctx, &opts, &node_name, &CloudOpts::route_s(), Some(&tcp)).await?;
    refresh_spaces(&ctx, &opts, &node_name, &CloudOpts::route_s(), Some(&tcp)).await?;
