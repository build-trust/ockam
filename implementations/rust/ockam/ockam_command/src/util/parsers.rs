...
pub(crate) fn identity_identifier_parser(input: &str) -> Result<IdentityIdentifier> {
    IdentityIdentifier::from_str(input)
        .map_err(|_| miette!("Invalid identity identifier: {input}").into())
}
