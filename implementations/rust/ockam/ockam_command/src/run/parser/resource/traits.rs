...
impl ParsedCommands {
    ...
    /// Create a list of parsed commands from just one command
    pub fn single<C: ParsedCommand + Send + 'static>(command: C) -> Self {
        ParsedCommands::new(vec![command])
    }

    /// Create an empty list of parsed commands
    pub fn empty() -> Self {
        ParsedCommands::new::<EmptyParsedCommand>(vec![])
    }
