...
if [ $# -eq 0 ]; then
  ...
  orchestrator_enrolled_suite=true
  ...
  orchestrator_enrolled_suite=false
...
for suite in "$@"; do
  ...
  orchestrator_enrolled) orchestrator_enrolled_suite=true ;;
...
if [ "$orchestrator_enrolled_suite" = true ]; then
  echo "Running orchestrator_enrolled suite..."
  bats "$current_directory/orchestrator_enrolled" --timing -j 3
fi
...
if [ "$kafka_suite" = true ]; then
  ...
  bats "$current_directory/kafka" --timing
fi
