...
@test "ebpf portals - create tcp outlet on implicit default node" {
  skip
  run_success "$OCKAM" node delete --all -y

  outlet_port="$(random_port)"
  run_success $OCKAM tcp-outlet create --ebpf --to "127.0.0.1:$outlet_port"
  assert_output --partial "/service/outlet"
}

@test "ebpf portals - create tcp outlet" {
  skip
  run_success "$OCKAM" node delete --all -y

  outlet_port="$(random_port)"
  run_success $OCKAM tcp-outlet create --ebpf --to "127.0.0.1:$outlet_port" --from "test-outlet"
...
@test "ebpf portals - tcp inlet CRUD" {
  skip
  ...
  outlet_port="$(random_port)"
  run_success $OCKAM tcp-outlet create --ebpf --at /node/n1 --to "127.0.0.1:$outlet_port"
  assert_output --partial "/service/outlet"

  inlet_port="$(random_port)"
  run_success $OCKAM tcp-inlet create --ebpf --at /node/n2 --from 127.0.0.1:$inlet_port --to /node/n1/service/outlet --alias "test-inlet"
  run_success $OCKAM tcp-inlet create --ebpf --at /node/n2 --from 6102 --to /node/n1/service/outlet
...
@test "ebpf portals - tcp outlet CRUD" {
  skip
  run_success "$OCKAM" node create n1
  ...
  port_1="$(random_port)"
  run_success $OCKAM tcp-outlet create --ebpf --at /node/n1 --to "127.0.0.1:$port_1"
  assert_output --partial "/service/outlet"

  port_2="$(random_port)"
  run_success $OCKAM tcp-outlet create --ebpf --at /node/n2 --to $port_2
...
@test "ebpf portals - list inlets on a node" {
  skip
  run_success "$OCKAM" node create n1
  run_success "$OCKAM" node create n2

  port="$(random_port)"
  run_success $OCKAM tcp-inlet create --ebpf --at /node/n2 --from $port --to /node/n1/service/outlet --alias tcp-inlet-2
  sleep 1
...
@test "ebpf portals - list outlets on a node" {
  skip
  run_success "$OCKAM" node create n1

  port="$(random_port)"
  run_success $OCKAM tcp-outlet create --ebpf --at /node/n1 --to "$port"
  assert_output --partial "/service/outlet"
...
@test "ebpf portals - show a tcp inlet" {
  skip
  run_success "$OCKAM" node create n1
  run_success "$OCKAM" node create n2

  port="$(random_port)"
  run_success $OCKAM tcp-inlet create --ebpf --at /node/n2 --from $port --to /node/n1/service/outlet --alias "test-inlet"
  sleep 1
...
@test "ebpf portals - show a tcp outlet" {
  skip
  run_success "$OCKAM" node create n1
...
@test "ebpf portals - create an inlet/outlet pair and move tcp traffic through it" {
  ...
  run_success "$OCKAM" tcp-outlet create --ebpf --at /node/n1 --to "$PYTHON_SERVER_PORT"
  port="$(random_port)"
  run_success "$OCKAM" tcp-inlet create --ebpf --at /node/n2 --from "$port" --to /node/n1/service/outlet

  run_success curl -sfI --retry-connrefused --retry-delay 5 --retry 10 -m 5 "127.0.0.1:$port"
}

@test "ebpf portals - create an inlet/outlet, download file" {
  run_success "$OCKAM" node create n1
  run_success "$OCKAM" node create n2
...
@test "ebpf portals - create an inlet/outlet pair and move tcp traffic through it, where the outlet points to an HTTPs endpoint" {
  ...
  run_success "$OCKAM" tcp-outlet create --ebpf --at /node/n1 --to google.com:443
  port="$(random_port)"
  run_success "$OCKAM" tcp-inlet create --ebpf --at /node/n2 --from "$port" --to /node/n1/service/outlet
...
@test "ebpf portals - create an inlet/outlet pair with relay through a relay and move tcp traffic through it" {
  ...
  run_success bash -c "$OCKAM secure-channel create --from /node/green --to /node/relay/service/forward_to_blue/service/api \
    | $OCKAM tcp-inlet create --ebpf --at /node/green --from $port --to -/service/outlet"
...
@test "ebpf portals - fail to create two TCP outlets with the same worker address" {
  skip
  run_success "$OCKAM" node create n

  port="$(random_port)"
  run_success "$OCKAM" tcp-outlet create --ebpf --at n --to "$port" --from o
  port="$(random_port)"
  run_failure "$OCKAM" tcp-outlet create --ebpf --at n --to "$port" --from o
}

@test "ebpf portals - fail to create two TCP inlets with the same alias" {
  skip
  run_success "$OCKAM" node create n

  port="$(random_port)"
  run_success "$OCKAM" tcp-outlet create --ebpf --at n --to "$port"

  port="$(random_port)"
  run_success "$OCKAM" tcp-inlet create --ebpf --at n --from "$port" --to "/node/n/service/outlet" --alias i
  port="$(random_port)"
  run_failure "$OCKAM" tcp-inlet create --ebpf --at n --from "$port" --to "/node/n/service/outlet" --alias i
}

@test "ebpf portals - fail to create two TCP inlets at the same socket address" {
  skip
  run_success "$OCKAM" node create n
  ...
  port="$(random_port)"
  run_success "$OCKAM" tcp-inlet create --ebpf --at n --from "$port" --to "/node/n/service/outlet"

  run_failure "$OCKAM" tcp-inlet create --ebpf --at n --from "$port" --to "/node/n/service/outlet"
}
...
@test "ebpf portals - local inlet and outlet, removing and re-creating the outlet" {
  ...
  run_success "$OCKAM" node create green
  inlet_port="$(random_port)"
  run_success "$OCKAM" tcp-inlet create --ebpf --at /node/green --from "$inlet_port" --to /node/blue/secure/api/service/outlet
...
@test "ebpf portals - local inlet and outlet in reverse order" {
  ...
  node_port="$(random_port)"
  inlet_port="$(random_port)"
  run_success "$OCKAM" tcp-inlet create --ebpf --at /node/n1 --from "${inlet_port}" --to "/ip4/127.0.0.1/tcp/${node_port}/service/outlet"

  run_success "$OCKAM" node create n2 --tcp-listener-address "127.0.0.1:${node_port}"
  run_success "$OCKAM" tcp-outlet create --ebpf --at /node/n2 --to "$PYTHON_SERVER_PORT"
...
@test "ebpf portals - local portal, curl download, inlet credential expires" {
  ...
  run_success "$OCKAM" tcp-outlet create --ebpf --at /node/bob --to "$PYTHON_SERVER_PORT"
  inlet_port="$(random_port)"
  run_success "$OCKAM" tcp-inlet create --ebpf --at /node/alice --from "$inlet_port" --to /node/bob/secure/api/service/outlet
...
@test "ebpf portals - create inlet with specific identifier" {
  skip
  run_success "$OCKAM" node create n
  alt=$("$OCKAM" identity create alt)
  run_success "$OCKAM" tcp-outlet create --ebpf --to "$PYTHON_SERVER_PORT" --allow "(= subject.identifier \"$alt\")"

  # Create an inlet with the node's identifier, without a secure channel. It shouldn't be allowed to connect
  port="$(random_port)"
  run_success "$OCKAM" tcp-inlet create --ebpf --from "127.0.0.1:$port" --to /node/n/service/outlet
  run_failure curl -sfI -m 3 "127.0.0.1:$port"

  # Same as before, but through a secure channel. It shouldn't be allowed to connect
  port="$(random_port)"
  run_success "$OCKAM" tcp-inlet create --ebpf --from "127.0.0.1:$port" --to /node/n/secure/api/service/outlet
  run_failure curl -sfI -m 3 "127.0.0.1:$port"

  # Create an inlet with the alt's identifier. Now it should be allowed to connect
  port="$(random_port)"
  run_success "$OCKAM" tcp-inlet create --ebpf --from "127.0.0.1:$port" --to /node/n/secure/api/service/outlet --identity alt
