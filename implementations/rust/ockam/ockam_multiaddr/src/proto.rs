...
impl Protocol<'_> for Mptcp {
    const CODE: Code = Code::new(7);
    const PREFIX: &'static str = "mptcp";

    fn read_str(input: Checked<&str>) -> Result<Self, Error> {
        u16::from_str(&input).map(Mptcp).map_err(Error::message)
    }

    fn read_bytes(input: Checked<&[u8]>) -> Result<Self, Error> {
        let mut b = [0; 2];
        b.copy_from_slice(&input);
        Ok(Mptcp(u16::from_be_bytes(b)))
    }
