use crate::{Challenge, Message, MessageGenerators, ALLOC_MSG, COMMITMENT_BYTES, FIELD_BYTES};
use bls::PublicKey;
...
pub struct PokSignatureProof {
    ...
    /// The fiat shamir challenge generated by the prover
...
impl PokSignatureProof {
    ...
    pub fn add_challenge_contribution<N>(
        &self,
        ...
        N: ArrayLength<G1Projective> + NonZero,
    {
        ...
        let mut proof1_scalars = [self.challenge.0, self.proofs1[0].0, self.proofs1[1].0];
        ...
        for (idx, msg) in rvl_msgs {
            hidden.insert(*idx);
            r_points.push(generators.h[*idx]).expect(ALLOC_MSG);
        ...
        proof2_scalars.push(self.challenge.0).expect(ALLOC_MSG);
        ...
        for i in 0..generators.h.len() {
            ...
            proof2_points.push(generators.h[i]).expect(ALLOC_MSG);
