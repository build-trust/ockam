...
use ockam_core::{
    errcode::{Kind, Origin},
    Address, AddressSet, AsyncTryClone, Error, LocalMessage, Mailboxes, Message, Processor,
    RelayMessage, Result, Route, TransportMessage, TransportType, Worker,
};
use ockam_core::{AccessControl, LocalInfo, Mailbox};
...
impl Context {
        ...
        AC: AccessControl,
    {
        let mailboxes = Mailboxes::main(
            Address::random_tagged(&format!(
                "{}.repeater",
                self.mailboxes.main_address().address()
            )),
            Arc::new(access_control),
        );

        let repeater_ctx = self.new_detached_impl(mailboxes).await?;
        debugger::log_inherit_context("REPEATER", self, &repeater_ctx);

        Ok(repeater_ctx)
    }

    /// Create a new detached `Context` that will apply the given
    /// [`AccessControlPair`] to any incoming messages it sends or receives
    pub async fn _unused_new_detached_with_access_control<IN, OUT>(
        &self,
        access_control: ockam_core::access_control::AccessControlPair<IN, OUT>,
        ...
        OUT: AccessControl,
    {
        let mailboxes = Mailboxes::main(
            ...
            Arc::new(access_control.incoming),
        );

        let repeater_ctx = self.new_detached_impl(mailboxes).await?;
        debugger::log_inherit_context("REPEATER_WITH_AC", self, &repeater_ctx);
    ...
    ///
    /// TODO this should require explicit `AccessControl` rather than
    /// inheriting the parent context's as this is rarely what one
    /// wants. See: `Self::new_detached_with_mailboxes`
    ...
    /// }
    /// ```
    pub async fn start_worker<NM, NW, S>(&self, address_set: S, worker: NW) -> Result<()>
    where
        S: Into<AddressSet>,
        NM: Message + Send + 'static,
        NW: Worker<Context = Context, Message = NM>,
    {
        WorkerBuilder::with_inherited_access_control(self, address_set, worker)
            .start(self)
            .await?;
        Ok(())
    }
        ...
        N: Message,
    {
        ...
        // TODO @ac 0#Context.send_and_receive.detached
        // in:  route
        // out: route
        // FIXME this fails some tests and needs to be debugged
        let mailboxes = Mailboxes::new(
            Mailbox::new(
                Address::random_tagged("Context.send_and_receive.detached"),
                //Arc::new(ockam_core::AllowSourceAddress(route.recipient())),
                //Arc::new(ockam_core::AllowDestinationAddress(route.next().unwrap().clone())),
                Arc::new(ockam_core::AllowAll), // FIXME
                Arc::new(crate::access_control::LocalOriginOnly), // FIXME
            ),
            vec![],
        );
        ...
        M: Message + Send + 'static,
    {
        ...
        // TODO check if the sender_address is allowed to send the message
