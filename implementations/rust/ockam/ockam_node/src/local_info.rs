...
use ockam_core::{Decodable, Encodable, LocalInfo, LocalMessage, Result, TransportType};
...
impl ExternalLocalInfo {
    ...
    pub fn from_local_info(value: &LocalInfo) -> Result<Self> {
        if value.type_identifier() != EXTERNAL_IDENTIFIER {
            return Err(Error::InvalidLocalInfoType.into());
        }
        ...
        Err(Error::InvalidLocalInfoType.into())
    }
