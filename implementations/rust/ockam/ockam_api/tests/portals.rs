...
async fn inlet_outlet_local_successful(context: &mut Context) -> ockam::Result<()> {
    ...
    let node_manager_handle = start_manager_for_tests(context, None).await?;
    ...
    let outlet_status = node_manager_handle
        ...
        .create_outlet(
            ...
            Address::from_string("outlet"),
            Some("outlet_alias".to_string()),
            true,
            Some(Arc::new(AllowAll)),
        )
        .await?;

    assert_eq!(outlet_status.alias, "outlet_alias");
    ...
    assert!(!inlet_status.outlet_route.is_empty());
...
impl TestNode {
    pub async fn create(runtime: Arc<Runtime>, listen_addr: Option<&str>) -> Self {
        let (mut context, mut executor) = NodeBuilder::new().runtime(runtime.clone()).build();
        runtime.spawn(async move {
            executor.start_router().await.expect("cannot start router");
        });
        let node_manager_handle = start_manager_for_tests(&mut context, listen_addr)
            .await
...
fn portal_connection_drop_packets() {
    ...
    //   - the portion of the received data matches whith the sent data.
    //

    test_portal_payload_transfer(Disruption::DropPacketsAfter(32));
}
...
fn portal_connection_change_packet_order() {
    ...
    test_portal_payload_transfer(Disruption::PacketsOutOfOrderAfter(32));
}

fn test_portal_payload_transfer(disruption: Disruption) {
    ...
    let result: ockam::Result<_> = runtime_cloned.block_on(async move {
        let test_body = async move {
            ...
            let passthrough_server_handle =
                start_passthrough_server(&second_node_listen_address.to_string(), disruption).await;
            ...
            let mut random_buffer: Vec<u8> = Vec::with_capacity(payload_size);
            random_buffer.resize(payload_size, 0);
            ...
            let size = rx.read_to_end(&mut incoming_buffer).await.unwrap();
            info!("size: {}", size);
            assert_ne!(size, 0);
