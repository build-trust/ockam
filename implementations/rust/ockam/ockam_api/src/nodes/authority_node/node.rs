...
pub async fn start_node(ctx: &Context, configuration: Configuration) -> Result<()> {
    ...
    let authority = Authority::create(ctx, configuration.clone()).await?;

    // start a secure channel listener (this also starts a TCP transport)
    authority
        .start_secure_channel_listener(ctx, configuration.clone())
        .await?;

    // start the authenticator services
    authority
        .start_direct_authenticator(ctx, configuration.clone())
        .await?;
    authority
        .start_enrollment_services(ctx, configuration.clone())
        .await?;
    authority
        .start_credential_issuer(ctx, configuration.clone())
        .await?;

    // start the Okta service (if the optional configuration has been provided)
    authority.start_okta(ctx, configuration.clone()).await?;
