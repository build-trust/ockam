...
impl NodeManager {
    ...
    pub(super) fn secure_channel_listener_list(
        &mut self,
        req: &Request<'_>,
    ) -> ResponseBuilder<SecureChannelListenerList> {
        Response::ok(req.id()).body(SecureChannelListenerList::new(
            self.registry
                .secure_channel_listeners
                .keys()
