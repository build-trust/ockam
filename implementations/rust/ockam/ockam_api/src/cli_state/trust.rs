...
impl CliState {
    ...
    #[instrument(skip_all, fields(project_name = project_name.clone(), authority_identity = authority_identity.as_ref().map(|a| a.to_log_string()).unwrap_or("n/a".to_string()), authority_route = authority_route.clone().map_or("n/a".to_string(), |r| r.to_string())))]
    pub async fn retrieve_trust_options(
        ...
        if let Some(authority_identity) = authority_identity {
            let identity_str = authority_identity.to_log_string();
            return self
