...
impl TcpPortalsRepository for TcpPortalsSqlxDatabase {
    ...
    async fn delete_tcp_portals_by_node(&self, node_name: &str) -> ockam_core::Result<()> {
        ...
        let query1 = query("DELETE FROM tcp_inlet WHERE node_name = ?").bind(node_name.to_sql());
...
impl TcpInletRow {
    fn bind_addr(&self) -> Result<SocketAddr> {
        Ok(SocketAddr::from_str(&self.bind_addr).map_err(|e| {
            ockam_core::Error::new(Origin::Api, Kind::Serialization, format!("{e:?}"))
        })?)
    }

    fn outlet_addr(&self) -> Result<MultiAddr> {
        Ok(MultiAddr::from_str(&self.outlet_addr).map_err(|e| {
            ockam_core::Error::new(Origin::Api, Kind::Serialization, format!("{e:?}"))
        })?)
    }
...
mod tests {
    ...
    async fn test_repository() -> Result<()> {
        ...
        // when a node is removed, all the corresponding inlets and outlets must be removed
        repository.store_tcp_inlet("node_name", &tcp_inlet).await?;
        repository
            .store_tcp_outlet("node_name", &tcp_outlet_status)
            .await?;

        repository.delete_tcp_portals_by_node("node_name").await?;
        let actual = repository.get_tcp_inlet("node_name", "alias").await?;
        assert_eq!(actual, None);
        let actual = repository.get_tcp_outlet("node_name", &worker_addr).await?;
        assert_eq!(actual, None);
