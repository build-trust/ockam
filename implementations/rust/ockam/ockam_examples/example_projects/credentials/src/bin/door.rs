...
use ockam::{
    credential_attribute_values, credential_type, route, Context, CredentialProtocol, Identity,
...
async fn main(ctx: Context) -> Result<()> {
    ...
    let mut entity = Identity::create(&ctx, &vault).await?;

    println!("Door id: {}", entity.identifier().await?);

    println!("Enter Office id: ");
    let office_id = read_line();
    let office_id = IdentityIdentifier::try_from(office_id.as_str())?;

    let tcp = TcpTransport::create(&ctx).await?;
    tcp.connect(OFFICE_TCP_ADDRESS).await?;

    // Just to get office's identity
    let _office_channel = entity
    ...
    entity
        .create_secure_channel_listener(DOOR_LISTENER_ADDRESS, TrustEveryonePolicy)
        .await?;
