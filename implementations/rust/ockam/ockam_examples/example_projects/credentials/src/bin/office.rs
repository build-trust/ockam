...
async fn main(ctx: Context) -> Result<()> {
    ...
    let mut entity = Identity::create(&ctx, &vault).await?; // TODO: add options to setup entity from creation
                                                            // Rename to create_credential_issuance_key
    entity
        .create_key(
            /* Move to Identity? */ Identity::CREDENTIALS_ISSUE.to_string(),
        )
        .await?;
    ...
    entity
        .create_secure_channel_listener(OFFICE_LISTENER_ADDRESS, TrustEveryonePolicy)
        .await?;

    println!("Enter Bob id: ");
    let bob_id = read_line();
    let bob_id = IdentityIdentifier::try_from(bob_id.as_str())?;

    entity
        .create_credential_issuance_listener(
