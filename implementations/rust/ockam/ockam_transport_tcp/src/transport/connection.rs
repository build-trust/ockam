...
impl TcpTransport {
    ...
    /// let addr = tcp.connect("127.0.0.1:5000", TcpConnectionTrustOptions::insecure()).await?; // and connect to port 5000
    /// # Ok(()) }
    /// ```
    pub async fn connect(
        ...
        trust_options.setup_session(addresses.receiver_address());
