...
#[cfg(test)]
mod test {
    use crate::message::{Address, Route};
    use std::convert::TryFrom;
    ...
    // #[test]
    // fn encoded_addr_from_str() {
    //     let s_addr = "127.0.0.1:8080";
    //     let a = IP4TcpAddress::try_from(s_addr).unwrap();
    //     let v = serde_bare::to_vec(&a).unwrap();
    //     assert_eq!(v[0..], [1, 7, 0, 127, 0, 0, 1, 144, 31]);
    // }
    //
    // #[test]
    // fn addr_from_encoded_str() {
    //     let encoded_addr = [1u8, 7, 0, 127, 0, 0, 1, 144, 31];
    //     let a = serde_bare::from_slice::<IP4TcpAddress>(&encoded_addr).unwrap();
    //     let comp_addr = IP4TcpAddress::try_from("127.0.0.1:8080").unwrap();
    //     assert_eq!(a, comp_addr);
    // }

    // #[test]
    // fn encoded_from_routable_addr() {
    //     let addr = RoutableAddress {
    //         addr_type: AddressType::Tcp,
    //         length: 7,
    //         address: Address::IP4TcpAddress(IP4TcpAddress::try_from("127.0.0.1:8080").unwrap()),
    //     };
    //     let v = serde_bare::to_vec::<RoutableAddress>(&addr).unwrap();
    //     println!("{:?}", v);
    // }
    //
    // #[test]
    // fn routable_address_from_encoded() {
    //     let encoded = [1u8, 7, 0, 127, 0, 0, 1, 144, 31];
    //     let at = serde_bare::from_slice::<AddressType>(&encoded[0..]).unwrap();
    //     println!("{:?}", at);
    //     let a = serde_bare::from_slice::<IP4TcpAddress>(&encoded[2..]).unwrap();
    //     println!("{:?}", a);
    //     let r = serde_bare::from_slice::<RoutableAddress>(&encoded);
    //     match r {
    //         Err(e) => {
    //             println!("{:?}", e);
    //         }
    ...
    //
    // #[test]
    // fn route_from_encoded_str() {
    //     let r_encoded = [1u8, 1u8, 7, 0, 127, 0, 0, 1, 144, 31];
    //     let r = Route::from_u8_slice(&r_encoded);
    //     if let Err(s) = r {
    //         println!("Route: {}", s);
    //     }
    // }
...
//             0 => MessageType::Ping,
//             1 => MessageType::Pong,
//             2 => MessageType::Payload,
//             3 => MessageType::KeyAgreementM1,
//             4 => MessageType::KeyAgreementM2,
//             5 => MessageType::KeyAgreementM3,
//             9 => MessageType::NoSuchChannel,
//             _ => MessageType::None,
//         }
//     }
// }
...
//             MessageType::KeyAgreementM1 => 3,
//             MessageType::KeyAgreementM2 => 4,
//             MessageType::KeyAgreementM3 => 5,
//             MessageType::NoSuchChannel => 9,
...
//         u.push(self.message_type.clone().into());
...
//             }
//         }
//         msg.message_type = MessageType::from(w[0]);
//         let mut w = &w[1..];
...
//     fn encode(&self, v: &mut Vec<u8>) -> Result<(), String> {
//         v.push(self.a_type.clone() as u8);
