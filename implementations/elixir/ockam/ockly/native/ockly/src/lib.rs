...
use ockam_vault::{PublicKey, Secret, SoftwareSigningVault, Vault};
...
fn create_identity(env: Env, existing_key: Option<String>) -> NifResult<(Binary, Binary)> {
    ...
    let identity = block_future(async move {
        if let Some(key) = existing_key {
            identities_ref
                .identities_creation()
                .create_identity_with_existing_key(&key)
                .await
        } else {
...
fn attest_purpose_key<'a>(
    ...
    let public_key = PurposePublicKey::SecureChannelStaticKey(k.try_into().unwrap()); // FIXME
    let (purpose_key, _data) = block_future(async move {
        identities_ref
            .purpose_keys()
            .attest_purpose_key(&identifier, public_key)
            .await
    })
    .map_err(|_| Error::Term(Box::new(atoms::attest_error())))?;
    let encoded = minicbor::to_vec(purpose_key)
...
#[rustler::nif]
fn verify_purpose_key_attestation(
    identity: Binary,
    public_key: Binary,
    attestation: Binary,
) -> NifResult<bool> {
...
fn issue_credential<'a>(
    ...
    .map_err(|reason| Error::Term(Box::new(reason)))?;
    let encoded = minicbor::to_vec(credential)
...
fn verify_credential(
    ...
    let attributes = block_future(async move {
        ...
        {
            attr_map.insert(
                String::from_utf8(k).map_err(|_| atoms::utf8_error())?,
                String::from_utf8(v).map_err(|_| atoms::utf8_error())?,
            );
        }
        Ok((
