defmodule Ockam.Stream.Storage.Kinesis do
  ...
  defp fetch_messages(
    ...
    case Kinesis.get_records(shard_iterator, limit) do
  ...
  defp fetch_messages(
    ...
    with {:ok, shard_iterator} <-
           Kinesis.get_shard_iterator(stream_name, partition, :after_sequence_number,
             starting_sequence_number: to_string(initial_sequence_number)
           ),
         {:ok, {records, next_shard_iterator, latest_index}} <-
  ...
  defp fetch_messages(stream_name, partition, index, limit, state) do
    with {:ok, shard_iterator} <-
      ...
      {{:ok, records},
       %{
         state
         | previous_index: index,
           previous_sequence_number: latest_index,
           next_shard_iterator: next_shard_iterator
       }}
    else
      error ->
