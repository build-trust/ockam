...
ockam_error_t ockam_key_initiate(ockam_key_t* p_key)
{
  ...
  if(!p_key) { error = KEYAGREEMENT_ERROR_PARAMETER; goto exit; }
  ...
  ockam_key_t* p_key, uint8_t* payload, size_t payload_size, uint8_t* msg, size_t msg_size, size_t* msg_length)
{
  ...
  if(!p_key || !payload || !msg || !msg_size) { error = KEYAGREEMENT_ERROR_PARAMETER; goto exit; }
  if(!payload_size || !msg_size )  { error = KEYAGREEMENT_ERROR_PARAMETER; goto exit; }
...
ockam_error_t ockam_key_decrypt(ockam_key_t* p_key,
                                uint8_t*              payload,
                                size_t                payload_size,
                                uint8_t*              msg,
                                size_t                msg_length,
