...
char* p_file_to_send =  "./test_data_client.txt";
char* p_file_to_receive = "./test_data_server.txt";
...
int main() {
	...
	while( receive_not_done ) {
		status = ockam_receive_blocking( connection, &receive_buffer[0], sizeof( receive_buffer ),
		                                 &bytes_received );
		if ( OCKAM_ERR_NONE != status ) {
