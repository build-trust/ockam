...
class Command {
    static async exec(command: string, args: string[]) {
        let out: string
        let errMsg: string
...
class Project {
    ...
    async enroll(attributes: string) {
        return await Command.exec("ockam", ["project", "enroll", "--attribute", attributes])
    }
    async authenticate(token: string, identity: Identity) {
...
class Ockam {
    ...
    async connectTunnel(ticket: string, port: string, relayOutlet: string) {
