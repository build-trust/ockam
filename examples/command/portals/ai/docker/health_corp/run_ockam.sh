...
#
# Create a TCP Portal Inlet to the ai API.
# This makes the remote postgres available on all localhost IPs at - 0.0.0.0:13000
ockam node create
ockam policy create --resource tcp-inlet --expression '(= subject.ai-outlet "true")'
until ockam tcp-inlet create --from 0.0.0.0:13000 --to ai; do sleep 10; done
