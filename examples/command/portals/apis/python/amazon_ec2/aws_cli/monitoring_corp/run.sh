...
run() {
    ...
    until scp -o StrictHostKeyChecking=no -i ./key.pem ./app.py "ec2-user@$ip:app.py"; do sleep 10; done
    ssh -o StrictHostKeyChecking=no -i ./key.pem "ec2-user@$ip" \
        'bash -s' << 'EOS'
            ...
            pip install flask
            ls . && echo "Dependencies installed."
            nohup flask run --host='0.0.0.0' &>output.log &
EOS
}
...
export name="ockam-ex-api-python-monitoring-corp-$user"
