...
#
# This hands-on example uses Ockam to create an end-to-end encrypted portal to postgres. We connect a nodejs app in
# one Amazon VPC with a Amazon Aurora managed Postgres database in another Amazon VPC.
#
...
run() {
    ...
    # project membership authority will cryptographically attest to the specified attributes - postgres-outlet=true.
    #
    ...
    #
    # The identity that enrolls with the generated ticket will be given a project membership credential in which the
    # project membership authority will cryptographically attest to the specified attributes - postgres-inlet=true.
