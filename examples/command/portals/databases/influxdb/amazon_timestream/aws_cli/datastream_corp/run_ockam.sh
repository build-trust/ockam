...
# attribute influxdb-outlet="true" to connect to TCP Portal Inlets on this node.
#
# Create a TCP Portal Inlet to influxdb.
# This makes the remote influxdb available on all localhost IPs at - 0.0.0.0:8086
cat << EOF > inlet.yaml
tcp-inlet:
  from: 0.0.0.0:8086
  via: influxdb
