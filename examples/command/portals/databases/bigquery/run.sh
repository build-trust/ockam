...
run() {
    ...
    # Create an enrollment ticket to enroll the identity used by an ockam node that will run adjacent to the bigquery
    ...
    # project membership authority will cryptographically attest to the specified attributes - bigquery-outlet=true.
    #
    # The identity will also allowed to create a relay in the project at the address `bigquery`.
    metrics_corp_ticket=$(ockam project ticket --usage-count 1 --expires-in 60m \
        --attribute "bigquery-outlet=true" --relay bigquery)

    # Create an enrollment ticket to enroll the identity used by an ockam node that will run adjacent to the node
    # that will access bigquery
    #  in datastream_corp's network.
    #
    # The identity that enrolls with the generated ticket will be given a project membership credential in which the
    # project membership authority will cryptographically attest to the specified attributes - bigquery-inlet=true.
    datastream_corp_ticket=$(ockam project ticket --usage-count 1 --expires-in 60m \
        --attribute "bigquery-inlet=true")
