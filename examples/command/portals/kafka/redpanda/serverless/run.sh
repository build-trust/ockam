...
run() {
    ...
    application_team_consumer_ticket=$(ockam project ticket --usage-count 1 --expires-in 10m --attribute consumer --attribute producer=false --relay '*')
    application_team_producer_ticket=$(ockam project ticket --usage-count 1 --expires-in 10m --attribute producer --attribute consumer=false)
...
check_parameters() {
    ...
    if [ -z "${REDPANDA_SASL_MECHANISM}" ]; then
        echo "Please set the REDPANDA_SASL_MECHANISM environment variable to the SASL mechanism of your Redpanda cluster. Either SCRAM-SHA-256 or SCRAM-SHA-512"
        exit 1
    fi
}
