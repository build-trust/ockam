...
run() {
    ...
    echo -e "#!/bin/sh\n"\
      "set -ex\n"\
      "ssh -o StrictHostKeyChecking=no -i ./client/key.pem 'ec2-user@$ip'"\
      "iperf3 --zerocopy --client 127.0.0.1 --port 15201 --time 60" > speed_test.sh
...
cleanup() {
    ...
    rm -f user_data.sh speed_test.sh
...
export name="ockam-ex-speed-test-client-$user"
