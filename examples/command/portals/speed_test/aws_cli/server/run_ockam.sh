...
#
# Create an encrypted relay to this node in the project at address: iperf.
...
# attribute iperf-inlet="true" to connect to TCP Portal Outlets on this node.
#
# Create a TCP Portal Outlet to iperf at - 127.0.0.1:5201.
OCKAM_LOG_LEVEL=info ockam node create
ockam relay create iperf
ockam policy create --resource-type tcp-outlet --expression '(= subject.iperf-inlet "true")'
ockam tcp-outlet create --to "127.0.0.1:5201"

sudo yum update -y
sudo yum install iperf3 screen -y

# start iperf server
screen -d -m iperf3 --server --bind 127.0.0.1 --port 5201
